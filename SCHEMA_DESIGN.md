# Heliolytics Schema — Design

> **Status:** Implemented. Canonical schema is [`schema.sql`](schema.sql); apply via
> `deploy/reset-db.sh` (full reset).
...
| DB migration | **Full reset OK** — wipe volume, apply `schema.sql` |
...
1. Land `schema.sql`; wire `docker-compose` / `reset-db.sh`

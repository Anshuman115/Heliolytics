...
func (h *metricsHandler) dailyHealthScores(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, map[string]any{"days": toHealthScoreTiles(rows)})
}
...
func toHealthScoreTiles(rows []store.DayMetric) []healthScoreTile {
	out := make([]healthScoreTile, 0, len(rows))
	for _, r := range rows {
		out = append(out, healthScoreTile{
			...
			AvgHr: r.AvgHr, TimeInBedMins: r.SleepMins,
		})
	}
	return out
}
